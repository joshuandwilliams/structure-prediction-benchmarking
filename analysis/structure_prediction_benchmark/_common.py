"""
Shared loading, constants and helpers for the benchmark analyses.

The analysis is split into several documents with one objective each.  All of
them need the same inputs and the same data-cleaning decisions, so those live
here rather than being repeated (and drifting) in each document.

Usage from a .qmd in this directory:
...
THESIS_FIGDIR = HERE / "thesis-figures"
SUPP_FIGDIR = HERE / "supplementary-figures"
...
def is_oracle(model):
    """True if this combo builds its input from the reference structure."""
...
def load(verbose=True):
    ...
    df = pd.read_csv(HERE / "combined_metrics.csv")
    ...
    pred = pd.read_csv(HERE / "per_prediction_metrics.csv")
    ...
    rt = pd.read_csv(HERE / "runtime_stats.csv")
