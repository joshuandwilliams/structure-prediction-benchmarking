 ...
 *
 * This module runs colabfold_search exactly once and emits a directory
 * containing chain_A.a3m and chain_B.a3m.  The workflow fans the result out
 * to BOLTZ1_MSA, BOLTZ2_MSA, and COLABFOLD via value channels so each
 * downstream process sees the same pre-computed MSAs.
 *
 * Container notes:
...
process COLABFOLD_SEARCH {
    ...
    script:
    """
    # ─── Write separate-style multimer FASTA ──────────────────────────────
    # colabfold_search produces one a3m per FASTA record, naming them after
    # the header.  Using "chain_A" / "chain_B" headers makes the output files
    # drop in directly as chain_A.a3m / chain_B.a3m.
...
FASTA_EOF
    ...
    if [ ! -f msa/chain_A.a3m ] || [ ! -f msa/chain_B.a3m ]; then
        echo "ERROR: Expected MSA files not found in msa/" >&2
        ls -la msa/ >&2
        exit 1
    fi

    echo "MSA files:"
    wc -l msa/chain_A.a3m msa/chain_B.a3m
    """
}
